from environment.environment import (
    ...
    CapsuleCollider,
    Cast,
    CastFrameChangeHolder,
    CasterPositionChange,
...
class RewardManager:
    ...
    def __init__(
        ...
        signal_subscriptions: Optional[Dict[str, Tuple[str, RewTerm]]] = None,
    ) -> None:
...
class SaveHandler:
        ...
        if self.mode == SaveHandlerMode.FORCE:
            ...
            if os.path.exists(exp_path) and len(os.listdir(exp_path)) != 0:
                while True:
                    answer = (
                        input(
                            f"Would you like to clear the folder {exp_path} (SaveHandlerMode.FORCE): yes (y) or no (n): "
                        )
                        .strip()
                        .lower()
                    )
    ...
    def _checkpoint_path(self, extension: str = "") -> str:
        ...
        return os.path.join(
            ...
            f"{self.name_prefix}_{self.num_timesteps}_steps.{extension}",
        )
...
class SelfPlayLatest(SelfPlayHandler):
    ...
    def get_opponent(self) -> Agent:
        assert (
            self.save_handler is not None
        ), "Save handler must be specified for self-play"
...
def run_match(
    ...
    for time in tqdm(range(max_timesteps), total=max_timesteps):
        ...
        full_action = {0: agent_1.predict(obs_1), 1: agent_2.predict(obs_2)}
